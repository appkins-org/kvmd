...
class Plugin(BaseUserGpioDriver):  # pylint: disable=too-many-instance-attributes
    ...
    async def __ensure_login(self) -> bool:
        if self.__csrf_token is None:
            ...
            filtered = session.cookie_jar.filter_cookies(self.__url)

            if "TOKEN" not in filtered:
                get_logger().info("Unifi API login required")
            elif filtered['TOKEN'].value != None:
                get_logger().info("Unifi API already logged in")
                return True
            ...
            try:
                async with session.post(
                    ...
                    if "X-CSRF-TOKEN" in response.headers:
                        ...
                        get_logger().error("Unifi API login failed, check credentials")
                        return False
                    if response.cookies:
    ...
    async def __handle_response(self, response: aiohttp.ClientResponse, is_login: bool = False) -> None:
